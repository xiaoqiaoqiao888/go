...
//
// This program used to import all illegal_mobile to Codis
//
...
func init() {
    ...
    log.AddFilter("file", l4g.WARNING, l4g.NewFileLogWriter(fmt.Sprintf("log/illegal_mobile_%d_%s.log", os.Getpid(), time.Now().Format("20060102150405")), false))
}
...
func worker(id int, jobs <- chan string, wg *sync.WaitGroup, redisPool *redis.Pool, pipelineJob int) {
    ...
    for j := range jobs {
        ...
        // 2 + 1(linenumber)
        if len(colList) != 3 {
            log.Error("worker %3d processing job: number of fields [%2d] does not equal 3", id, len(colList))
        ...
        // colList[0] = mobile_no
        var args = []interface{}{ "IllegalMob:1" + strings.TrimSpace(colList[0]) }
