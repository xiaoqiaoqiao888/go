...
//
// This program used to import all super_user to Codis
//
...
func init() {
    ...
    log.AddFilter("file", l4g.WARNING, l4g.NewFileLogWriter(fmt.Sprintf("log/super_user_%d_%s.log", os.Getpid(), time.Now().Format("20060102150405")), false))
}
...
func worker(id int, jobs <- chan string, wg *sync.WaitGroup, redisPool *redis.Pool, pipelineJob int) {
    ...
    for j := range jobs {
        ...
        // 3 + 1(linenumber)
        if len(colList) != 4 {
            log.Error("worker %3d processing job: number of fields [%2d] does not equal 4", id, len(colList))
        ...
        // colList[0] = register_name
        var args = []interface{}{ "Super:" + strings.TrimSpace(colList[0]) }

        for i := 0; i < len(colList); i++ {
            colList[i] = strings.TrimSpace(colList[i])

            // reformat the lost_time
            switch i {
            case 2:
