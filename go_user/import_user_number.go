...
//
// This program used to import all user_number to Codis
//
...
func init() {
    ...
    log.AddFilter("file", l4g.WARNING, l4g.NewFileLogWriter(fmt.Sprintf("log/user_number_%d_%s.log", os.Getpid(), time.Now().Format("20060102150405")), false))
}
...
func worker(id int, jobs <- chan string, wg *sync.WaitGroup, redisPool *redis.Pool, pipelineJob int) {
    ...
    var fieldsName = []string{
        "a", // define_user_number
        "b", // define_user_limit_time
        "c", // real_user_count
    }
